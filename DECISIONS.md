...
**The light follows the ritual's time of day, and holds still.** The room opens at a time of day chosen at random from those the room offers, through `chooseAtmosphere`, and `Sky/DaylightCycle.ts` turns that time into the sun's position, its warmth and the lights' strength, so the light is testable in Node, and `Views/RoomLights.ts` applies it. A sun that crossed the sky during play was set aside for now, because its shadow edges crept across the room in visible steps. The time of day is dawn, day or sunset, and the presentation picks a random hour within it in `RoomMain.ts`: dawn from 0.8 to 2.2 hours after sunrise, day from 3 to 8, sunset from 10 to 11.4. So each visit has its own shade of light without a named stage for each. No rule depends on the time of day yet. The sun rises in the east behind the window wall, stands over that wall at noon, and turns in the afternoon until it faces the open front on the left, so the setting sun shines into the room and things cast shadows. A sun that set in the west behind the left wall was rejected, because no sunlight reached the room and nothing cast a shadow. Dawn starts at 0.8 hours for the same reason: earlier, the sun is still below the window wall. The noon light stays warm and a little softer than full sun, because a cold white light at noon made the room look like a hospital ward.

**The garden around the house is scenery.** `GardenLayout.ts` places every plant from a fixed seed, only outside the room's floor, so the garden is the same at every visit and never grows through the room. The plants are scattered 12 metres out from the house with room around each one, and each flower picks its kind and colour on its own, because beds looked like blocks of colour and the flowers in them stood on top of each other. `Views/Garden.ts` draws each part of a kind of plant as one instanced mesh, the plants cast no shadows and take no taps, so a thousand plants cost little on a phone. Each part of a plant is a flat-shaded shape of 3 to 20 faces, because the garden drew over 200 thousand triangles a frame for flowers a few pixels wide, and now draws about 64 thousand. The sunflower's model stays in `Views/Garden.ts` and is not planted for now, because the user will choose a place for it.
