...
**The garden around the house is scenery.** `GardenLayout.ts` places every plant from a fixed seed, only outside the room's floor, so the garden is the same at every visit and never grows through the room. The plants are scattered 12 metres out from the house with room around each one, and each flower picks its kind and colour on its own, because beds looked like blocks of colour and the flowers in them stood on top of each other. `Views/Garden.ts` draws each part of a kind of plant as one instanced mesh, the plants cast no shadows and take no taps, so a thousand plants cost little on a phone. The sunflower's model stays in `Views/Garden.ts` and is not planted for now, because the user will choose a place for it.
