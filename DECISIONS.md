...
**The garden around the house is scenery.** `GardenLayout.ts` places every plant from a fixed seed, only outside the room's floor, so the garden is the same at every visit and never grows through the room. The plants are scattered over the whole garden with room around each one, and each flower picks its kind and colour on its own, because beds looked like blocks of colour and the flowers in them stood on top of each other. `Views/Garden.ts` draws each part of a kind of plant as one instanced mesh, the plants cast no shadows and take no taps, so a thousand plants cost little on a phone. The tall sunflowers stand off to the sides or behind the house and face the camera, so they never hide the room.
