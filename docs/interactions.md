...
| Fill from the tap | open the kettle's lid, choose its hand, then tap the tap; tap it again to close it | `startFillingFromTap`, `stopFillingFromTap` | `fillingStarted`, `vesselOverflowed`, `fillingFinished` with filled and overflowed millilitres |
...
| Tap the tap or the sink under it with a hand chosen | The keeper holds that hand's vessel under the tap, and the water runs if the vessel's lid is open. With the lid closed nothing runs: the player opens the lid first, by tapping it. The kettle's water gauge rises. A second tap on the tap, the sink or the vessel under the tap closes it. |
