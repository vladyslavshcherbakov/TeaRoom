...
GitHub Actions runs `.github/workflows/test-and-deploy.yml` on every push, and on every pull request from a fork: a pull request from a branch of this repository is tested by the push of that branch. Three jobs run side by side: `Scripts/test.sh`, `Scripts/test-ui.sh` once for each browser, and `Scripts/build.sh`. On a push to the default branch, once all three pass, it publishes `dist/` to GitHub Pages at <https://vladyslavshcherbakov.github.io/TeaRoom/>.
