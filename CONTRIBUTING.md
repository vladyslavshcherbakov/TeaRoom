...
- `Room/RoomLayout.ts` holds every position in metres. `roomLayoutFor` builds the `RoomLayout` of an arrangement from where `Shared/Content/Rooms.ts` places the counter, the shelf and the tea table, and from the shape of each piece. `Room/RoomArrangement.ts` holds the arrangement a new game chooses at random: the window, which of the kitchen and the shelf stands beside it, the tea table's place, where the tools lie, the cushions and the cloths. `Shared/Content/Rooms.ts` builds the room's definition for an arrangement, and `RoomMain.ts` opens that room and saves the arrangement with the visit.
- `Room/CarriedShapes.ts` gives each carried item its shape and holds what each shape takes up on a surface.
- `Room/RoomPlay.ts` is the only place that turns presses into ritual commands: taking, choosing a hand, putting down, lids, the heater, the sink and its tap, pouring, the spoon, sipping, offering and wiping. It hands walking and close-ups to `Room/RoomNavigator.ts`, an aimed pour to `Room/AimedPour.ts`, a stroke of the cloth to `Room/WipeStroke.ts`, and counts taps in a row with `Room/TapsInARow.ts`. `Room/SipGesture.ts` times a sip: how far the vessel is raised to the lips and how much of the sip is still shown in it. What the player does that the room may remark on goes to `Room/RoomRemarks.ts` as a fact, and its `remarksInOrder` decides which remark the fact earns, and whether a remark is made once a visit. A new remark is one entry there. One switch in `RoomPlay`, `closeUpActionOn`, says what a tap in a close-up does, whether the chosen item does it, and whether it is a control. A control, the heater's switch or the tap, works whatever the hands hold, and a tap reaches it through the chosen hand's item. An item goes somewhere only by a tap on that place with its hand chosen: the surface, the heater or the sink.
...
- `Room/Placement.ts` decides whether an item fits where the player tapped. An item standing on furniture is turned by `turnOfItemAt` in `Room/RoomLayout.ts`, from the turn kept in its spot or the front of its furniture, and whatever places or draws it uses that turn.
- `Room/Temperatures.ts` turns degrees Celsius into the whole degrees the player chose to see, and steps the thermostat's target one of those degrees.
...
- `Room/Walking/` finds paths on the floor grid and moves the walker. `Room/Camera/` computes where the camera looks and keeps the zooms, and `Camera/FirstPersonLook.ts` turns the sticks and the mouse into steps and a look in first person, `Camera/KeeperHeight.ts` holds the keeper's height and the eyes' height standing or sitting at the tea table, and `Camera/FirstPersonControls.ts` holds the four control schemes, which sticks each one shows and how the keys walk. `Views/KeyboardAndMouse.ts` listens to the keys and catches the mouse. `Room/KeyboardShortcuts.ts` turns the keys 1, 2, 3, E and Space into taps and holds on the hands, a sip and the tilt of a pour. `Room/Sky/` computes where the sun stands at each hour of the day.
- `Room/Views/` builds the meshes. `RoomModel.ts` builds the room. `CarriedItems.ts` builds the items the keeper can carry, placed from the ritual's state every frame and, in a close-up, held in the corners of the view, with their parts in `Views/Carried/`: one parts file per shape, such as `KettleParts.ts` and `BowlParts.ts`, each with the shape's `CarriedShapeLook`: how its model is built, where its steam rises and how large its puffs are, how it shows leaves and where it burns, assembled by `CarriedModel.ts`, with the parts' `levelsOfDetail`, a simpler geometry for a costly part, or none, that `CarriedItems.ts` draws while the item is small on the screen, what an item holds, the water streams, the chosen glow, the veil over what the other hands hold, the fire of an item burning on the heater and the ash of one that crumbles. `Views/Carried/InspectedInView.ts` places an item shown up close in front of the camera, and `Views/InspectionStage.ts` dims the room behind it and lights it. `Views/HeaterControls.ts` builds the panel under the heater, with the thermostat's arrows and button in nerd mode, and `Views/LampDisplay.ts` paints the row of nixie tubes it shares with the kettle's thermometer. `SipButton.ts`, `PourControls.ts`, `Joysticks.ts`, `DebugMenu.ts` and `RoomCaption.ts` are the controls, the debug menu with the keeper's height, the frame budget, googly eyes, a giant afro, every face at once and filling the kettle with boiling water, whose pupils roll as `Room/GooglyPupil.ts` says, kept for good by `Room/DebugSettingsStore.ts` apart from the visit, and the caption, and an arrow held down, the thermostat's or the debug menu's, repeats as `Room/HeldArrow.ts` says, `Sky.ts` is the sky of first person, and `RoomLights.ts` lights the room from the sun's place.
- `Views/RoomGlow.ts` adds a bloom around what glows: a mesh marked with `markAsGlowing` in `Views/RoomLayers.ts`, a material marked there whose emissive colour then glows, or an additive material. A new lamp is marked there, with a strength below 1 when what it shows must stay readable. `Room/Views/RoomMaterials.ts` is the one place that decides how each surface looks. Its `lookBySurface` gives every surface its colour and the kind of material it is made of, so a new surface is one entry there. The garden's plants are lit more simply, and `plantMaterialFor` gives each part of a plant its colour from `colourByPlantSurface`. Paintings and generated textures, such as `KoiPainting.ts` and `LotusPainting.ts`, are drawn on canvases and used there.
...
- `RoomScene.ts` renders, raycasts and forwards pointer events to `RoomGestures`, and saves the visit through `Room/VisitStore.ts`. `Room/Achievements.ts` decides which events, remarks and states earn an achievement, `Room/AchievementStore.ts` keeps them in the browser, and `Views/AchievementsList.ts` and `Views/AchievementNotice.ts` show them. `Views/ProphecySighting.ts` tells whether the prophecy on the beam is seen whole. `Views/GuideBook.ts` shows the book under the gear, the room's instructions, one part per spread, `Views/GuideBookModel.ts` builds the folio on the wall, and `Views/GuidePagePainting.ts` paints the faint writing on its pages. `Room/PlayTime.ts` counts the time the room is played, and `Room/PlayTimeStore.ts` keeps it in the browser. `Room/RoomSettings.ts` holds the player's settings, `Room/SettingsStore.ts` keeps them in the browser, and `Views/SettingsScreen.ts` shows them. Every store reaches the browser's storage only through `Room/BrowserStorage.ts`, which turns a blocked or full storage into a result the store logs. `Views/EdgeSmoothing.ts` smooths the edges of the finished picture when the player asks for it. `Room/FrameBudget.ts` times each part of a frame, and `Views/FrameBudgetPanel.ts` shows its report with the renderer's counts when the debug menu asks. A new part of the frame gets a phase there. `Room/FrameRate.ts` counts the frames the browser draws, and `Views/FrameRateCounter.ts` shows the count. `Views/FullScreenButton.ts` puts the page in full screen on a computer, and `Views/LeaveFirstPersonButton.ts` goes back from first person to the room view. Both stand in one row in the top right corner, so they never overlap. `RoomMain.ts` opens the ritual session, or resumes a saved one after `Views/ContinueScreen.ts` asks, which also lists the keys on a computer.
...
- `FittedSavedState.ts` decides whether a saved state still fits the game. A new or renamed field is one more entry at the end of its `migrationsOldestFirst`, with a test that resumes a save from before it. A change it cannot see, such as a new meaning for an existing field, bumps `sessionStateVersion`. `Room/VisitStore.ts` keeps its own `migrationsOldestFirst` for the visit around the state.
...
- Every action on an item first checks that it is within reach. `Ritual/ItemRefusals.ts` holds the checks that actions share, each with its refusal reason and the values it logs, and an action lists the ones it needs in its own order with `wasRefusedByAnyOf`. An action that already holds the item's location asks `Ritual/Reach.ts` directly. Every action with the spoon or the cloth checks that it is in a hand, except soaking up a puddle, where the cloth lies on the surface. Offering checks that the keeper stands at the ritual place, and wiping that the keeper stands at a place with a puddle. A new action gets the same checks.
