...
class RitualBench {
  ...
  private heatingSection(): HTMLElement {
    return section(
      ...
      row(
        button('Sink: put kettle in', () => this.send({ type: 'putInTheSink', itemId: 'kettle' })),
        button('Tap: turn on', () => this.send({ type: 'turnTheTapOn' })),
        button('Tap: turn off', () => this.send({ type: 'turnTheTapOff' })),
      ),
