...
export class RoomScene {
  ...
  private frame(): void {
    ...
    const realSeconds = this.clock.getDelta()
    ...
    this.shortcuts.advance(realSeconds)
    this.debugMenu.advance(realSeconds)
    this.gestures.advance(realSeconds)
    this.frameBudget.phaseEnded('input', performance.now())
    this.play.advance({ worldSeconds, realSeconds })
    this.frameBudget.phaseEnded('roomPlay', performance.now())
    this.reactTo(this.session.advance(worldSeconds))
    ...
    this.keepTheVisitNowAndThen(realSeconds)
    this.playTime.frameDrawn(realSeconds, { isThePageShown: document.visibilityState === 'visible', hasTheKeeperDied: this.hasTheKeeperDied })
    ...
    this.carried.show({ state, table, walk: this.play.walk, heldInView, inspected, aimedPour: this.play.aimedPourView, clothWiping: this.play.clothWiping, sipGesture: this.play.sipGestureView, timeSeconds: this.clock.elapsedTime, temperatureUnitShown: this.settings.isNerdModeOn ? this.settings.temperatureUnit : null, distantDetail: this.settings.objectDetail === 'reduced' ? { camera: this.camera, screenHeightPixels: window.innerHeight } : null })
