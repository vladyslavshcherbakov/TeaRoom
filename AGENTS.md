...
- Text the player reads is not in the simulation. The simulation emits ids such as remark names, and the presentation turns them into words. Every such word lives in `Apps/Game/Texts/EnglishTexts.ts` as a key and a value, and code reads it through `Texts.ts`. No player-facing string is written anywhere else. The bench is a debug page and keeps its own labels.
