...
export function startFillingFromTap(draft: Draft, command: CommandOfType<'startFillingFromTap'>): void {
  ...
  if (vesselDefinitionOf(draft, vessel).lid?.mustBeOpenToFill === true && !vessel.isLidOpen) return refuse(draft, command, 'lidClosed')
  draft.state.filling = { vesselId: vessel.id, filledMl: 0, overflowedMl: 0, hasOverflowed: false }
  note(draft, `tap opened over ${describeLiquid(vessel)}, water at ${tap.waterTemperatureC} °C, ${tap.flowMlPerSecond} ml/s`)
...
export function finishFilling(draft: Draft, reason: string): void {
  ...
  note(
    ...
    `filling ${filling.vesselId} stopped because ${reason}: ${filling.filledMl.toFixed(1)} ml in, ` +
      `${filling.overflowedMl.toFixed(1)} ml over the rim into the sink, now ${vessel === undefined ? 'gone' : describeLiquid(vessel)}`,
  )
